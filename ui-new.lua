...
local PERF_MODE = (_G.SofrUiPerfMode ~= nil) and _G.SofrUiPerfMode or true

local gameName = "Place_" .. tostring(game.PlaceId)
if not PERF_MODE then
...
function SaveConfig()
    if not writefile or not ConfigFile then return end
    if not CURRENT_VERSION then return end

    local seq = (ConfigData._saveSeq or 0) + 1
    ConfigData._saveSeq = seq

    if ConfigData._saveWorkerRunning then return end
    ConfigData._saveWorkerRunning = true

    task.spawn(function()
        while true do
            local last = ConfigData._saveSeq
            task.wait(0.25)
            if ConfigData._saveSeq == last then
                break
            end
        end
        ConfigData._saveWorkerRunning = false
...
local function MakeDraggable(topbarobject, object)
    local function CustomPos(topbarobject, object)
        ...
        local function UpdatePos(input)
            ...
            tween(object, TweenInfo.new(0.2), { Position = pos })
        end
    ...
    local function CustomSize(object)
        ...
        local function UpdateSize(input)
            ...
            tween(object, TweenInfo.new(0.2), { Size = UDim2.new(0, newWidth, 0, newHeight) })
        end
...
function PandoruyHub:MakeNotify(NotifyConfig)
    ...
    task.spawn(function()
        ...
        if not CoreGui.NotifyGui:FindFirstChild("NotifyLayout") then
            ...
            CoreGui.NotifyGui.NotifyLayout.ChildRemoved:Connect(function()
                ...
                for i, v in CoreGui.NotifyGui.NotifyLayout:GetChildren() do
                    tween(v, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
        ...
        function NotifyFunction:Close()
            ...
            waitbruh = true
            tween(NotifyFrameReal, TweenInfo.new(tonumber(NotifyConfig.Time), Enum.EasingStyle.Back, Enum.EasingDirection.InOut), {
...
function PandoruyHub:Window(GuiConfig)
    ...
    DropShadow.ImageTransparency = PERF_MODE and 1 or 0.5
    ...
    LayersPageLayout.TweenTime = PERF_MODE and 0 or 0.5
    ...
    local function UpdateSize1()
        ...
        local curSeq = tabSeq
        task.wait()
        if curSeq ~= tabSeq then return end
    ...
    Fullscreen.Activated:Connect(function()
        ...
        if isFullscreen then
            ...
            tween(DropShadowHolder, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                Size = originalSize,
    ...
    ConnectButton.Activated:Connect(function()
        if MoreBlur.Visible then
            tween(MoreBlur, TweenInfo.new(0.3), { BackgroundTransparency = 0.999 })
            tween(DropdownSelect, TweenInfo.new(0.3), { Position = UDim2.new(1, 262, 0.5, 0) })
            if not PERF_MODE then
                task.wait(0.3)
            end
            MoreBlur.Visible = false
    ...
    DropPageLayout.TweenTime = PERF_MODE and 0 or 0.009999999776482582
    ...
    function Tabs:AddTab(TabConfig)
            ...
            if FrameChoose ~= nil and Tab.LayoutOrder ~= LayersPageLayout.CurrentPage.LayoutOrder then
                for _, TabFrame in ScrollTab:GetChildren() do
                    if TabFrame.Name == "Tab" then
                        tween(TabFrame, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.InOut), {
                            BackgroundTransparency = 0.9990000128746033
                        })
                    end
                end
                tween(Tab, TweenInfo.new(0.6, Enum.EasingStyle.Back, Enum.EasingDirection.InOut), {
                    BackgroundTransparency = 0.9200000166893005
                })
                tween(FrameChoose, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
                ...
                NameTab.Text = TabConfig.Name
                tween(FrameChoose, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
                    Size = UDim2.new(0, 1, 0, 20)
                })
                if not PERF_MODE then
                    task.wait(0.2)
                end
                tween(FrameChoose, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
        ...
        function Sections:AddSection(Title, AlwaysOpen)
            ...
            local function UpdateSizeScroll()
                ...
                if layout then
                    ScrolLayers.CanvasSize = UDim2.new(0, 0, 0, layout.AbsoluteContentSize.Y)
                end
            end
            ...
            local function UpdateSizeSection()
                ...
                local curSeq = sectionSeq
                task.wait()
                if curSeq ~= sectionSeq then return end

                if OpenSection then
                    local SectionSizeYWitdh = 38 + UIListLayout2.AbsoluteContentSize.Y
                    tween(FeatureFrame, TweenInfo.new(0.5), { Rotation = 90 })
                    tween(Section, TweenInfo.new(0.5), { Size = UDim2.new(1, 1, 0, SectionSizeYWitdh) })
                    tween(SectionAdd, TweenInfo.new(0.5), { Size = UDim2.new(1, 0, 0, SectionSizeYWitdh - 38) })
                    tween(SectionDecideFrame, TweenInfo.new(0.5), { Size = UDim2.new(1, 0, 0, 2) })
                    if not PERF_MODE then
                        task.wait(0.5)
                    end
                    UpdateSizeScroll()
            ...
            if AlwaysOpen ~= true then
                SectionButton.Activated:Connect(function()
                    ...
                    if OpenSection then
                        tween(FeatureFrame, TweenInfo.new(0.5), { Rotation = 0 })
                        tween(Section, TweenInfo.new(0.5), { Size = UDim2.new(1, 1, 0, 30) })
                        tween(SectionDecideFrame, TweenInfo.new(0.5), { Size = UDim2.new(0, 0, 0, 2) })
                        OpenSection = false
                        if not PERF_MODE then
                            task.wait(0.5)
                        end
            ...
            function Items:AddToggle(ToggleConfig)
                ...
                function ToggleFunc:Set(Value)
                    ...
                    SaveConfig()
                    if Value then
                        tween(ToggleTitle, TweenInfo.new(0.2), { TextColor3 = GuiConfig.Color })
                        tween(ToggleCircle, TweenInfo.new(0.2), { Position = UDim2.new(0, 15, 0, 0) })
                        tween(UIStroke8, TweenInfo.new(0.2), { Color = GuiConfig.Color, Transparency = 0 })
                        tween(FeatureFrame2, TweenInfo.new(0.2), {
                            BackgroundColor3 = GuiConfig.Color,
                            BackgroundTransparency = 0
                        })
                    else
                        tween(ToggleTitle, TweenInfo.new(0.2), { TextColor3 = Color3.fromRGB(230, 230, 230) })
                        tween(ToggleCircle, TweenInfo.new(0.2), { Position = UDim2.new(0, 0, 0, 0) })
                        tween(UIStroke8, TweenInfo.new(0.2), {
            ...
            function Items:AddSlider(SliderConfig)
                ...
                function SliderFunc:Set(Value)
                    ...
                    tween(SliderDraggable, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                ...
                SliderFrame.InputBegan:Connect(function(Input)
                    if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                        Dragging = true
                        tween(SliderCircle, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            ...
            function Items:AddDropdown(DropdownConfig)
                ...
                DropdownButton.Activated:Connect(function()
                    if not MoreBlur.Visible then
                        ...
                        tween(MoreBlur, TweenInfo.new(0.3), { BackgroundTransparency = 1 })
                        tween(DropdownSelect, TweenInfo.new(0.3), { Position = UDim2.new(1, -11, 0.5, 0) })
                    end
                end)
                ...
                SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
                    ...
                    local curSeq = searchSeq
                    task.wait(0.1)
                    if curSeq ~= searchSeq then return end
                ...
                function DropdownFunc:Set(Value)
                    ...
                    for _, Drop in ScrollSelect:GetChildren() do
                        if Drop.Name == "Option" and Drop:FindFirstChild("OptionText") then
                            ...
                            if selected then
                                tween(Drop.ChooseFrame, TweenInfo.new(0.2), { Size = UDim2.new(0, 1, 0, 12) })
                                tween(Drop.ChooseFrame.UIStroke, TweenInfo.new(0.2), { Transparency = 0 })
                                tween(Drop, TweenInfo.new(0.2), { BackgroundTransparency = 0.935 })
                                table.insert(texts, Drop.OptionText.Text)
                            else
                                tween(Drop.ChooseFrame, TweenInfo.new(0.1), { Size = UDim2.new(0, 0, 0, 0) })
                                tween(Drop.ChooseFrame.UIStroke, TweenInfo.new(0.1), { Transparency = 0.999 })
                                tween(Drop, TweenInfo.new(0.1), { BackgroundTransparency = 0.999 })
                            end
                        end
                    end
